...
- **Deep-link URL**: new `keycloak_console_url: str | None` config field (`backend/app/core/config.py`), same "browser-facing override, falls back to the internal URL" pattern as `tracing_ui_url` vs `tracing_otlp_endpoint` — `keycloak_url` (`http://keycloak:8080` in docker-compose, a `.svc.cluster.local` address in the Helm chart) is not resolvable from an admin's browser. Rides along on the existing `GET /api/settings/dashboard` response (`app.services.dashboard_service.resolve_dashboard_status`) rather than a new endpoint, since that's the response the admin nav already fetches. `docker-compose.yml` sets `KEYCLOAK_CONSOLE_URL=http://localhost:8080` explicitly for the backend service (previously silently missing — the backend's `KEYCLOAK_URL` there was already an internal-only address with no browser-facing counterpart set anywhere); the Helm chart gained an analogous `keycloakConsoleUrl` value (`infra/helm/charts/backend/values.yaml`), left unset by default since it can't be derived from the sibling `keycloak` chart's own `externalHostname` (Helm subcharts can't see each other's values, only `global`).
...
- Also produced this session: a process addition to `AGENTS.md`'s TDD Discipline step 0 requiring every new-feature plan/PR to state explicitly whether it's a user feature or an admin feature (`require_admin`-gated) — prompted by planning this issue, on the observation that RBAC is fully wired in this repo but nothing forced that call to be made and written down up front.
