...
**Admin User-Management Surface Complete ✓**: Keycloak Deep Link + Docs (issue #40)
- EAIStack delegates identity/authorization entirely to Keycloak, but had no in-product way to answer "who has access" or onboard/offboard a teammate — an operator had to independently know Keycloak exists and find its admin console. A new "User Management" nav entry (admin-gated, same `isAdmin` pattern as Settings/Dashboard/Audit Log) is a pure external deep link into Keycloak's own admin console, opening directly to the configured realm's user list in a new tab — not a new in-app screen.
- **Scope decision**: a full in-app user list/role-editor (calling Keycloak's Admin REST API from the backend) was considered and deliberately not built. It would require granting the backend's Keycloak service account `manage-users` — a meaningfully larger trust surface — and would duplicate audit capability Keycloak already provides natively. The issue's own stated minimum bar (deep link + docs) was shipped instead; the fuller editor, if ever picked up, deserves its own issue and its own look at that trust-surface trade-off at that time.
- **Deep-link URL, composed server-side**: new `keycloak_console_url: str | None` config field (`backend/app/core/config.py`) is the browser-facing Keycloak root — `keycloak_url` (`http://keycloak:8080` in docker-compose, a `.svc.cluster.local` address in the Helm chart) is not resolvable from an admin's browser. Deliberately does **not** fall back to `keycloak_url` when unset: an earlier version of this change did, and code review caught that a Helm deployment which forgets to set the override would silently render a nav link pointed at an unreachable internal address. `None` means "not configured" and the frontend correctly omits the nav link. `app.services.nav_config_service.resolve_nav_config` also composes the *full* URL (root + `keycloak_realm` + admin-console path) server-side, not in the frontend, so the realm name is resolved in exactly one place instead of being duplicated as a hardcoded literal that could drift if a fork renames its realm. Served by its own lightweight `GET /api/settings/nav-config` endpoint — deliberately **not** piggybacked on `GET /api/settings/dashboard`, since that endpoint does real work (a 24h audit-log aggregation query, live rate-limit bucket introspection) that a static nav-config value shouldn't force every admin login to pay for. `docker-compose.yml` sets `KEYCLOAK_CONSOLE_URL=http://localhost:8080` explicitly for the backend service (previously silently missing); the Helm chart gained an analogous `keycloakConsoleUrl` value (`infra/helm/charts/backend/values.yaml`), left unset by default since it can't be derived from the sibling `keycloak` chart's own `externalHostname` (Helm subcharts can't see each other's values, only `global`) — left unset there, the nav link is now correctly absent rather than broken.
- **Keycloak's own audit trail, not EAIStack's**: `infra/keycloak/realm-import.json` (and the matching Helm chart realm-import ConfigMap) now enable Keycloak's built-in Admin Events and Login Events by default (`eventsEnabled`, `adminEventsEnabled`, `adminEventsDetailsEnabled`) — a fresh `docker-compose up` or Helm install has them on without any manual step. This is deliberately Keycloak's own event store, not a write into EAIStack's `audit_logs` table — see `docs/USER_MANAGEMENT.md` for why, and [issue #63](../../../issues/63) for the deferred follow-up considering a read-only proxy view of those events inside EAIStack's own Audit Log screen.
- New `docs/USER_MANAGEMENT.md`: a zero-prior-Keycloak-knowledge guide to creating a user, granting/removing the `admin` role, and the disable-vs-delete distinction (neither purges EAIStack data immediately — cross-references `docs/REPOSITORY_PATTERN.md`'s user-isolation model and `docs/SECURITY.md`'s retention policy).
- A related, separately-shipped process change (not part of this PR's diff — see the `chore/rbac-process-check` branch): `AGENTS.md`'s TDD Discipline step 0 now requires every new-feature plan/PR to state explicitly whether it's a user feature or an admin feature (`require_admin`-gated), prompted by planning this issue on the observation that RBAC is fully wired in this repo but nothing forced that call to be made and written down up front.

**End-User Retention Notice Complete ✓**: Chat-UI Transparency for Data Retention (issue #49)
- Retention windows were fully admin-configurable since Phase 4b, but visibility into them was admin-only — an ordinary chat user had no in-product way to see how long their own conversation history was kept. **User feature** (not admin-gated): any authenticated user, not just an admin, can now see the effective retention policy governing their own data.
- New `GET /api/settings/retention-notice` (`get_current_user`, not `require_admin`) reports the same *effective* (resolved DB-override-or-env-default) values `GET /api/settings` already exposed to admins for `conversation_retention_hours`/`cleanup_on_logout` — deliberately not admin-gated, since these are read-only already-resolved values, not the editable admin config. `ChatWindow` renders them as a small, dismissible notice (`RetentionNotice.tsx`) in plain language ("retained for N hours" / "kept indefinitely" / "purged immediately", plus a note when logout-triggered cleanup is also on).
- **Configuration call-out**: `retention_notice_enabled` (new admin knob, env-default `True` — transparent by default, DB-override via `backend/app/services/retention_notice_config_service.py`, resolved per-call like `audit_log_ui_enabled`) lets a fork hide the notice entirely (e.g. every window is "keep forever" and it would be uninteresting). Exposed on the Settings screen with a tooltip; changing it is audit-logged (`retention_notice.config_update`), same pattern as every other admin-configurable switch.
- Dismissal is per-session only (in-memory component state, not persisted) — deliberately no localStorage/backend persistence for "seen this before," since AGENTS.md's no-premature-abstraction guidance argues against building that until a real need for it shows up.
- Verified end-to-end against the real stack (`docker-compose up --profile llm`): `frontend/tests/e2e/retention-notice.spec.ts` confirms the notice renders with a real resolved value and can be dismissed. Content-independent (no LLM interaction needed), so this spec runs under CI's default fake-provider profile like most of the suite.
