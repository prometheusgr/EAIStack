...
Mirrors backend/app/core/tls.py (doc-search intentionally does not share a
package with the backend, per the project's established split — see
app/auth.py and app/config.py's docstrings). Phase 5 (Decision 2) mounts
the internal CA's ca.crt into the pod and points every outbound httpx client
at it. This module has the one expression that implements that rule
(`httpx_verify()`) and a process-wide cache of the parsed SSL trust store
(`get_ssl_context()`), so callers stop re-parsing the same PEM file on every
request.
"""
...
def get_ssl_context() -> ssl.SSLContext:
    ...
    generate_query_embedding builds a fresh httpx.Client per knowledge-base
    query, so it uses this instead of `httpx_verify()`. The JWKS fetch in
    app/auth.py already caches its whole AsyncClient as a process-wide
    singleton (`_get_http_client`), so it continues to call `httpx_verify()`
    directly — there is no per-call context to cache there.
    """
    global _ssl_context
