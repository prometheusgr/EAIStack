...
**For new features**:
0. Name the user-visible signal before writing the spec test: what will a person actually see, receive, or be able to do differently because of this feature (a new screen or control, changed text/data in an existing one, an error message that now appears, a response header/status the frontend surfaces, a field in an admin screen)? If the honest answer is "nothing, ever" then either the scope is incomplete — a backend capability with no way for a user to reach or observe it usually needs a UI/response counterpart — or that gap is being deliberately deferred to a tracked follow-up issue; either way, decide and write it down before step 1, not after the code is done. This is what step 1's test should specify, not just the internal mechanism.
   - **RBAC call-out, in the same breath as the user-visible signal**: state explicitly whether this feature is a **user feature** (any authenticated user) or an **admin feature** (gated by `require_admin`, same as `Settings`/`Audit Log`/`Dashboard`). Say so in the plan/issue and in the PR description, not just implicitly via which endpoint the code happens to call. If access should differ by role in some other way, say what that is. This is a one-line decision, not a full RBAC redesign — the point is to force the choice to be made and written down before implementation starts, not discovered afterward by whoever notices an admin-only capability was left reachable by any logged-in user (or vice versa).
   - **Configuration call-out, for every new numeric/boolean/threshold/limit the feature introduces**: for each one, decide and write down whether it is (a) a **SystemSettings-backed admin knob** (env-default + nullable-DB-override, resolved per-call via a `resolve_*_config` function, exposed on the Settings UI with a tooltip, audit-logged on change — the pattern already used for retention, guardrails, tracing, rate-limiting, LLM/embedding provider config), or (b) a **deliberate fixed constant**, named and given a one-line rationale comment for why it's *not* a setting (e.g. schema-tied like `EMBEDDING_DIMENSION`, a restart-only value where DB-override adds nothing over an env var, a security-sensitive value that shouldn't be DB-editable, or a genuine implementation detail no operator would reason about directly). What is not acceptable is a third, silent option: an unnamed magic number an operator discovers only by reading source. This call-out is what would have caught chunk size/overlap, retrieval top_k, the missing similarity-relevance threshold, MCP/JWKS timeouts, the LLM `temperature` param, and the output-guardrail leak-detection threshold all shipping as undiscoverable constants instead of a documented choice — see the RAG/embedding configuration audit (2026-09) for the concrete list this pattern already missed once.
1. Write a failing test that specifies the desired behavior — including the user-visible signal named in step 0, not just the underlying logic
...
5. If the feature is user-facing (a new UI flow, or a change to an existing one), add or update an e2e test against the real running stack once the UI exists (see "End-to-End (E2E) Tests" below) — this step comes after green, not before, since there's normally no working UI to drive until the feature is built. The e2e test should exercise the same user-visible signal identified in step 0.

**For bugs**:
0. Name the user-visible signal: what will a person see differently once this is fixed (an error message instead of a silent failure or a generic 500, correct data instead of wrong, a control that now works)? A fix with no observable difference to any user is a sign the wrong root cause was found, not a legitimate silent fix.
1. Write a failing test that reproduces the bug — asserting on the user-visible signal from step 0, not just the internal state that was wrong
2. Run the test and confirm it fails (red)
...
- **A stale DB-persisted admin override from a previous local session can silently make the fake-provider suite run real inference.** `resolve_llm_config`/`resolve_tracing_config` resolve a DB-stored `SystemSettings` row *before* the env var — and that row lives in a named Postgres volume that survives `docker-compose down`/`up`/`build`, unlike the env var itself. A `docker-compose up --profile llm` session (or manually poking the Settings screen) that leaves `llm_provider="llama-cpp"` behind means the next "fake provider" suite run silently pays for real, CPU-bound inference on every chat turn — multiple seconds per LLM call instead of instant — which reads as unrelated flaky timeouts (a rate-limit burst test, a guardrail send-and-wait assertion) with no obvious link to its actual cause. `tests/e2e/global-setup.ts`'s `resetStaleAdminOverrides` closes this automatically: before every local or CI run, it logs in as the seeded `testuser` (via Keycloak's password grant against the `eaistack-api` confidential client, not a browser) and `PUT /api/settings`s `llm_provider`/`llm_url`/`llm_model`/`tracing_enabled` back to `null` (env defaults), best-effort (a failure here only warns, never blocks the suite). No action needed to benefit from this — it runs on every e2e invocation.
...
- [ ] **The PR description states how this change is observable to the end user**, carrying forward the user-visible signal named at step 0 of the TDD Discipline above (see "For new features" / "For bugs") — this is a check that the signal identified before coding started actually shipped, not a new analysis done after the fact. If step 0 concluded "none, purely internal," say so and why; if what shipped no longer matches what step 0 predicted, that's a sign of scope drift worth calling out explicitly. This is what caught rate limiting (#25) shipping a `429` response with no corresponding UI treatment, and should be the first thing that prevents a repeat.
